/* Round 2, attempt 2.  The framebuffer-object harness read back nothing at
   all, including its control, so Apple's CARenderer does not render into a
   bound FBO.  This gives the CGL context a real drawable instead, a pbuffer,
   and renders the same cases.  The control is a plain layer with a green
   background: if that reads back empty again the harness is wrong, not the
   compositor, and nothing here may be recorded.

   Run on a macOS runner:
     clang -framework QuartzCore -framework CoreGraphics -framework Foundation \
       -framework OpenGL Tests/oracle/qc_replicator3_probe.m -o qc_replicator3_probe
*/
...
static int startGL(void)
{
  ...
  printf("pixel format %d, %d formats\n", (int)err, (int)npix);
  ...
  err = CGLCreatePBuffer(W, H, GL_TEXTURE_2D, GL_RGBA, 0, &pbuf);
  printf("pbuffer %d\n", (int)err);
  if (err != kCGLNoError || pbuf == NULL)
    {
      return 0;
    }
  ...
  CGLSetCurrentContext(ctx);
  {
    ...
    err = CGLSetPBuffer(ctx, pbuf, 0, 0, screen);
    printf("attach %d\n", (int)err);
    if (err != kCGLNoError)
      {
        return 0;
  ...
  @autoreleasepool
    {
      if (!startGL())
        {
          printf("no drawable, nothing measured\n");
          return 0;
        }
